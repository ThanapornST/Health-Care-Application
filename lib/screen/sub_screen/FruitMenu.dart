...
import 'package:healthcares/data/dummy_Fruit.dart'; 

class FruitManuScreen extends StatefulWidget {
  const FruitManuScreen({super.key});
...
class _FruitManuScreenState extends State<FruitManuScreen> {
  final List<int> _itemCounts = List.generate(dummyFruit.length, (index) => 0);

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Fruit Manu'),
        backgroundColor: const Color.fromARGB(255, 77, 63, 44),
      ),
      body: ListView.builder(
        itemCount: dummyFruit.length, 
        itemBuilder: (ctx, index) {
          final menu = dummyFruit[index]; 
          return InkWell(
            child: Card(
